...
test('deadline locks atomically; reveal embargo, audited results, correction and final history',async()=>{
 ...
 assert.equal(value(await s.call('GET',base)).board,undefined);assert.equal(value(await s.call('GET',base)).commentary,undefined);assert.equal(value(await s.call('GET',base)).live,undefined);
 s.advance();
 assert.equal((await s.call('PUT','/api/contests/week/me/pick-card',{...s.card,expectedCardRevision:1},p.session)).statusCode,409);
 let view=value(await s.call('GET',base));assert.equal(view.contest.phase,'REVEAL');assert.deepEqual(view.reveal.groups,[]);assert.equal(view.board,undefined);assert.equal(view.commentary.id,'locked');assert.equal(view.live,undefined);
