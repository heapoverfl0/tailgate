 ...
 return <div className="live-stage">
  ...
  <div className="live-columns">
   <section className="live-rank-panel"><div className="panel-title"><h2>Standings</h2><span>{board.entries.length} PLAYING</span></div>
    <table className="live-rank-table"><thead><tr><th scope="col">PLAYER</th><th scope="col">PTS</th><th scope="col">PROJ</th><th scope="col" title="Projected rank compared with banked rank">Δ RANK</th></tr></thead><tbody>{board.entries.map(e=>{const delta=e.rank-(e.projectedRank??e.rank);return <tr key={e.participantId} className={e.rank===1?'rank-leading':''}><th scope="row"><span className="rank-number">{e.rank}</span><span>{e.displayName}{e.attendance==='REMOTE'&&<small className="remote-tag">REMOTE</small>}</span></th><td>{e.points}</td><td className="projected-number">{e.projectedPoints??e.points}</td><td className={delta>0?'rank-up':delta<0?'rank-down':'rank-flat'} aria-label={delta===0?'Projected rank unchanged':`Projected rank ${delta>0?'up':'down'} ${Math.abs(delta)}`}>{delta>0?'↑':delta<0?'↓':'—'}{delta!==0&&Math.abs(delta)}</td></tr>})}</tbody></table>
    ...
    <div className="rank-footnote"><span>THE RULE IS SIMPLE.</span><p>Make your picks.<br/><em>Own the consequences.</em></p></div>
   </section>
   <section className="featured-panel"><div className="panel-title"><h2>Game to watch</h2><span>{featured?`${featured.stakes} PICK POINTS UNRESOLVED`:'BETWEEN GAMES'}</span></div>
    {featured?<><div className="featured-score"><div><span>{featured.awayTeamId}</span><strong>{featured.fact?.away??'—'}</strong></div><div><span>{featured.homeTeamId}</span><strong>{featured.fact?.home??'—'}</strong></div><p><b className="live-dot">LIVE</b> {clock(featured.fact)} {featured.fact?.clock&&<span>· observed clock</span>}</p></div>
