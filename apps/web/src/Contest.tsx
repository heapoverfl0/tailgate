...
export function Contest({ id }: { id: string }) {
  ...
  return <main className={display ? 'display' : view.contest.phase==='MAIN_EVENT'?'main-event-page':''}><div className="eyebrow">{locked ? 'PICKS CLOSED' : `LOCKS IN ${Math.floor(minutes/60)}H ${minutes%60}M`} · {id}</div><h1>{view.contest.name}</h1><p className="intro">{locked ? (['LIVE','MAIN_EVENT','FINAL'].includes(view.contest.phase) ? 'Picks are public. Follow the standings below.' : 'Your card is frozen. Picks remain private until the reveal.') : 'Your picks stay private. Submit when complete; edit until lock.'}</p>
    ...
    {!display && card && view.contest.phase!=='MAIN_EVENT' && <section className="pick-card"><h2>Your card <small>{card.submissionStatus === 'SUBMITTED' ? 'Submitted' : 'Draft'}</small></h2>
