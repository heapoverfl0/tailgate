...
export function Reveal({id,config,view,admin,busy,advance,error,shared}:{id:string;config:ContestConfiguration;view:DayView;admin:boolean;busy:boolean;advance:()=>void;error:string;shared:boolean}){
 ...
 <header className="reveal-hero"><span className="eyebrow">{r.step===0?'NO MORE SECOND GUESSING':'THE CARDS ARE ON THE TABLE'}</span><h1>{titles[r.step]??r.title}</h1>{(r.step===0||r.step===5)&&<p>{r.step===0?'Let’s see what these idiots did.':'Five picks. One last chance to be insufferable.'}</p>}</header>
