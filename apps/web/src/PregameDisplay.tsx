...
export function PregameDisplay({id,view,now,joinUrl,qrAvailable,error}:{id:string;view:View;now:number;joinUrl:string;qrAvailable:boolean;error:string}) {
 ...
 <section className="pregame-slate"><h2>Today’s slate</h2>{id==='sept12-2026-final'&&<small>AP Top 25 · Week 2, 2026</small>}{[...windows].sort((a,b)=>a[1].order-b[1].order).map(([window,group])=><section className="pregame-window" key={window}><h3>{window}</h3><div>{group.games.map(g=>{const props=view.configuration.propositions.filter(p=>p.gameId===g.id);const cats=[...new Set(view.configuration.slots.filter(s=>s.choices.some(ch=>ch.kind==='PROPOSITION_OUTCOME'&&props.some(p=>p.id===ch.propositionId))).map(s=>categories[s.category]))];const lines=props.flatMap(p=>p.parameters.kind==='AGAINST_SPREAD'?[`${p.parameters.favoredTeamId} ${p.parameters.spread>0?'+':''}${p.parameters.spread}`]:p.parameters.kind==='GAME_TOTAL'?[`Total ${p.parameters.total}`]:[]);return <article key={g.id}><strong>{rankedTeam(g.awayTeamId,view.configuration)} <span>at</span> {rankedTeam(g.homeTeamId,view.configuration)}</strong><small>{cats.join(' · ')}</small>{lines.length>0&&<p>{rankedLabel(lines.join(' · '),view.configuration)}</p>}</article>})}</div></section>)}</section>
